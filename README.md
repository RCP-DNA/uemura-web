...
- **現在のURL**: `uemura-web-53ym.vercel.app`
- **用途**: 開発・テストサーバーとして使用。開発の一段落後に新VPSへ移行予定

#### 従来の本番サーバー（Creative Japan - 旧VPS）

- **サーバー**: qmiweb11.creative-japan.org (140.227.126.196)
- **OS**: CentOS 6.10（EOL、カーネル2.6.32）
- **現状**: Docker非対応のため、リバースプロキシ構築は見送り
- **方針**: 開発はVercelで進め、一段落後に新VPSへ移行
- **注意**: 旧サーバーは現状維持。新VPS移行時に廃止予定
...
- **2025年12月**: 開発方針変更 - Vercelを開発・テスト環境として使用。旧VPS（CentOS 6）でのDocker構築は見送り。開発一段落後に新VPSへ移行予定
